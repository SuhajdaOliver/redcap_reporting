...
def kitoltottseg_felvetel_havi(token, url="https://redcap.onkobank.com/redcap/api/"):
    ...
    records["felveteli_datum_ido"] = pd.to_datetime(records["felveteli_datum_ido"], errors="coerce")

    records["felvetel_month"] = (records["felveteli_datum_ido"].dt.to_period("M").astype(str))
    ...
    for form, fields in required_by_form.items():
        ...
        results.append(temp[["record_id", "redcap_data_access_group", "form_name", "filled_required", "total_required", "felvetel_month"]])

    completion = pd.concat(results, ignore_index=True)

    summary = (completion.groupby(["felvetel_month", "redcap_data_access_group", "form_name"])
    .agg(records=("record_id", "count"), filled_required=("filled_required", "sum"), total_required=("total_required", "sum"))
    .reset_index()
    )
